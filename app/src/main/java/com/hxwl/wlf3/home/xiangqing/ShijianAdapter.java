...
public class ShijianAdapter extends RecyclerView.Adapter<ShijianAdapter.ViewHolder> {
    ...
    private ArrayList<Pinlin3Bean.DataBean.ReplyListBean> listBeans=new ArrayList<>();
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        GlidUtils.setGrid2(context, URLS.URL+list.get(position).getHeadImg(),holder.pinlun_touxiang);
        ...
        if (list.get(position).getReplyList().size()>=3){
        ...
        listBeans.clear();
        listBeans.addAll(list.get(position).getReplyList());
        ShijianZIAdapter adapter= new ShijianZIAdapter(context,listBeans,position);
        ...
        holder.zan.setOnClickListener(new View.OnClickListener() {
            ...
            private void quxiaozan(final ImageView zan, final Pinlin3Bean.DataBean dataBean) {
                OkHttpUtils.post()
            ...
            private void dianzan(final ImageView zan, final Pinlin3Bean.DataBean dataBean) {
                OkHttpUtils.post()
                        .url(URLS.ADDFAVOUR)
                        .addParams("targetId",dataBean.getId())
                        .addParams("token", MakerApplication.instance.getToken())
