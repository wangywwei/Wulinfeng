package com.hxwl.common.errorlog;
...
public class TheAppCrashHandler implements UncaughtExceptionHandler {
	// 单例
	...
	public void init(Context context) {
		...
		sp = context.getSharedPreferences("Bug_Log", context.MODE_PRIVATE);
...
//			try {java.lang.IllegalStateException: Expected a string but was BEGIN_ARRAY at line 1 column 24 path $.data
//				Log.e(TAG, "sleep");
//				Thread.sleep(3000);
...
//			}
	...
	private boolean handleException(Throwable ex) {
		...
		try {
			...
			final String strErrorMsg = GsonUtil.GsonString(errorMsg); // yukun时间：5-23 17：20改了 原因是没用fastjson框架 用gson实现
			if(sp != null){
				String bugText = sp.getString("buglog","");
				if(TextUtils.isEmpty(bugText)){
					sp.edit().putString("buglog",errorinfo).commit();
				}else{
					sp.edit().putString("buglog",bugText+"====bug分割===="+errorinfo).commit();
				}
			}
...
//		setSartActivity();
		...
		for (ActivityManager.RunningAppProcessInfo amProcess : mRunningProcess) {
			if (amProcess.processName.equals("com.hxwl.wulinfeng")) {
