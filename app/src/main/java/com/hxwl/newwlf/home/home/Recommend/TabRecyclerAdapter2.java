...
public class TabRecyclerAdapter2 extends RecyclerView.Adapter<TabRecyclerAdapter2.ViewHolde> {
    private Context context;
    private ArrayList<HomeRecommendBean.DataBean.SplendidVideoListBean> list;

    public void setList(ArrayList<HomeRecommendBean.DataBean.SplendidVideoListBean> list) {
        this.list = list;
        notifyDataSetChanged();
    }


    public TabRecyclerAdapter2(Context context, ArrayList<HomeRecommendBean.DataBean.SplendidVideoListBean> list) {
    ...
    @Override
    public TabRecyclerAdapter2.ViewHolde onCreateViewHolder(ViewGroup parent, int viewType) {
    ...
    @Override
    public void onBindViewHolder(TabRecyclerAdapter2.ViewHolde holder, final int position) {
        ...
        Glide.with(context).load(URLS.IMG+list.get(position).getPublicityImage()).error(R.drawable.wlf_deimg)
                .into(holder.id_index_gallery_item_image);
